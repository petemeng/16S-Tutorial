...
![](../assets/16s/ch13_publication_overview-6640b2f071.png)
...
# A tibble: 15 × 2
 ...
 2 Filtered ASVs                                848
 3 Dominant phylum                              Actinobacteriota (48.73%)
 4 LEfSe significant genera                     10
 5 ANCOM-BC significant genera                  0
 6 Consensus biomarkers                         0
 7 PICRUSt2 significant pathways                3
 8 NSTI median                                  0.219
 9 Spearman network edges                       1335
10 Network modularity index                     0.44
11 Best RF LOOCV ROC AUC                        0.797
12 SourceTracker mean unknown (Baquedano sinks) 0.933
13 SourceTracker mean unknown (Yungay sinks)    0.993
14 Mantel r                                     0.383
15 Procrustes correlation                       0.805
```
...
# A tibble: 10 × 11
 ...
 1 WD2101_soil_group                     7.41      Yungay      3.65 TRUE      Yungay        0.548  FALSE        27    104           2
 2 Paenibacillus                         5.96      Baquedano   3.80 TRUE      Baquedano    -1.10   FALSE         2      0           2
 3 0319-7L14                             9.94      NA          NA   NA        Yungay        1.09   FALSE        62   1158           1
 4 Rubrobacter                           7.31      NA          NA   NA        Yungay        1.11   FALSE        42     76           1
 5 Ammoniphilus                          5.63      NA          NA   NA        Baquedano    -0.757  FALSE        14    135           1
 6 Unclassified_Family_unclassified.10   8.71      NA          NA   NA        NA            NA     NA           NA     NA           0
 7 Unclassified_Family_unclassified.16   7.88      NA          NA   NA        NA            NA     NA           NA     NA           0
 8 Unclassified_Family_unclassified.12   7.81      NA          NA   NA        NA            NA     NA           NA     NA           0
 9 Unclassified_Xanthobacteraceae        7.65      NA          NA   NA        NA            NA     NA           NA     NA           0
10 Unclassified_Family_unclassified.1    7.58      NA          NA   NA        NA            NA     NA           NA     NA           0
```

这张表的意义非常直接：

**`WD2101_soil_group`** 和 **`Paenibacillus`** 是整个系列里证据最集中的核心 biomarker 候选（EvidenceCount = 2）——前者 LEfSe 显著且在 ASV 网络中 degree = 27，后者 LEfSe 显著且在网络中也有连接。

`0319-7L14` 是 ASV 共现网络中的绝对核心（degree = 62, betweenness = 1158），同时也是随机森林重要性排名第一的特征。虽然它在差异分析中只在 ANCOM-BC 中有方向性信号但未达显著阈值，但在网络拓扑和分类预测两个维度上都是最突出的。
...
2. **差异层面**：LEfSe 检出 11 个候选 genus，ANCOM-BC 未检出显著差异属；LEfSe + RF + 网络汇聚最强的候选 biomarker 是 `Paenibacillus`
3. **网络层面**：ASV 水平共现网络识别出 20 个模块（modularity = 0.44），hub ASV 以 `0319-7L14` 等 Actinobacteriota 为主（degree 最高 62），全部 1335 条边为正相关
4. **分类层面**：随机森林 LOOCV AUC 为 0.797
