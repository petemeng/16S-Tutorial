...
26 条显著边里正相关占多数（18 vs 8），正负比约 2.25:1。这在微生物共现网络中是常见的——协同变化关系通常多于竞争/排斥关系。
...
模块化分析：
  模块数: 4
  Modularity index: 0.598
  各模块节点数: 6, 5, 5, 7
```

Modularity index = 0.598，说明网络模块结构非常清晰。通常 M > 0.4 就认为有明显的模块结构，这里远超这个阈值。

4 个模块的节点数分别是 6、5、5、7，分布比较均匀，不存在一个巨型模块吃掉全部节点的情况。
...
当前这个网络只有 23 个节点、26 条边，规模较小，所有节点都落在 Peripheral 象限。这是完全合理的——经典 Zi-Pi 分类在节点数 > 100 的大网络中更容易筛出 keystone taxa。

但 Zi-Pi 图仍然能看出哪些节点更接近 hub 阈值：

![](../assets/16s/ch09_zipi_plot-54bda56164.png)

**图 1：Zi-Pi 拓扑角色分类图。** 虚线为经典阈值（Zi = 2.5, Pi = 0.62），将节点分为四个象限。当前网络中所有节点落在 Peripheral 象限，但 `Ralstonia`（Pi = 0.50, Zi = 1.26）、`67-14`（Pi = 0.38, Zi = 1.58）和 `Unclassified_Xanthobacteraceae`（Pi = 0.38, Zi = 1.57）最接近 hub 区域，提示它们在更大数据集中有可能成为 keystone taxa。
...
![](../assets/16s/ch09_network_modularity-b6c55c2b43.png)

**图 2：Atacama 土壤 genus 共现网络。** 节点颜色编码模块归属（4 个模块），节点大小编码 degree，红色边为正相关，蓝色边为负相关，边宽编码相关系数绝对值。布局使用 Fruchterman-Reingold 力导向算法。`Ralstonia`（Module 4, degree = 6）是 degree 最高的核心节点，`67-14`（Module 1）和 `Unclassified_Xanthobacteraceae`（Module 3）紧随其后。
...
# A tibble: 12 × 2
 ...
 3 Positive edges                 18
 4 Negative edges                 8
 5 Positive:Negative ratio        2.25
 6 Average degree                 2.26
 7 Network diameter               10
 8 Average path length            3.96
 9 Graph density                  0.1028
10 Average clustering coefficient 0.108
11 Modularity index               0.598
12 Number of modules              4
```

几个值得注意的指标：

1. **Modularity = 0.598** — 模块结构非常显著（> 0.4）
2. **正负边比 = 2.25** — 协同关系多于竞争关系
3. **Graph density = 0.103** — 网络较稀疏，这在微生物共现网络中是正常的
4. **Average clustering coefficient = 0.108** — 网络局部聚集性不强，没有大量三角闭合结构
...
![](../assets/16s/ch09_network_combined-5b3caeb94d.png)
...
**"在 genus 水平经 prevalence/丰度过滤后，共保留 33 个特征用于网络分析。CLR 转换后的 Spearman 共现网络包含 23 个节点和 26 条显著边（18 条正相关、8 条负相关）。Fast greedy 模块化检测识别出 4 个模块（modularity index = 0.598）。Zi-Pi 拓扑角色分类显示所有节点均为 Peripheral 类型，但 `Ralstonia`（Zi = 1.26, Pi = 0.50）在 degree 和 betweenness 上均排名第一，提示其在 Atacama 土壤微生物共变结构中占据核心位置。"**

这样写的好处是：

1. 不会把"共现"说成"互作已证实"
2. 交代了模块化分析和 Zi-Pi 分类的结果
3. 正确处理了"小网络无 keystone"这个客观限制
4. 给后面随机森林和整合分析留下接口
...
**网络构建**：CLR + Spearman，26 条显著边（18 正/8 负）。

**模块化检测**：fast greedy 算法识别出 4 个模块，modularity = 0.598，模块结构显著。

**Zi-Pi 分类**：所有节点为 Peripheral（网络规模较小），但 `Ralstonia`（degree = 6, betweenness = 156）、`67-14`（degree = 4, betweenness = 142）和 `Candidatus_Udaeobacter`（degree = 3, betweenness = 100）是最接近 hub 区域的候选。
