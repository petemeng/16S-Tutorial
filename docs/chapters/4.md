...
``` 
📊 输出：
-rw-rw-r-- 1 user user 209M data/ref/silva-138-99-nb-classifier.qza
```
...
**剩下的 21.8% 呢？** 它们要么被注释到了更高的分类水平（比如只到科或目），要么分类器的置信度太低被截断了。这不意味着这些 ASV 是"错误"的——它们可能是参考数据库中缺少近缘序列的未知物种，或者是 V4 区序列在多个属之间无法区分的类群。
...
![](assets/atacama/phylum-composition-transect.png)
...
过滤掉了约 1 个线粒体/叶绿体来源的 ASV。这很符合土壤样本的预期：我们不是在做叶面或根际微生物组，所以植物来源污染并不严重，但这一步仍然值得保留，因为它能保证后续分析的对象更干净。
...
从 1,080 → 1,079（去线粒体/叶绿体后）→ 848（去低频 ASV，min-frequency=10）。相比 min-frequency=2（仅去除单次出现），min-frequency=10 能更有效地去除潜在的测序噪声，最终保留 848 个高置信度的 ASV 用于下游分析。
