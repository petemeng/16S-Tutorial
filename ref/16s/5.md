...
![](assets/atacama/alpha-rarefaction-observed-features.png)
...
> 📊 **本章使用的数据版本：抽平后的 ASV 表**
>
> `core-metrics-phylogenetic` 命令会先将 `table-filtered.qza`（未抽平，54 个样本）按指定深度随机抽样，生成 `rarefied_table.qza`（抽平后，47 个样本）。**后续所有 alpha 和 beta 多样性指标都基于这张抽平表计算。** 抽平深度不够的 7 个样本会被丢弃。
>
> 为什么多样性分析要用抽平数据？因为 Shannon、observed features 等指标对测序深度非常敏感——不抽平的话，深度高的样本天然得分更高（见上节稀释曲线）。
...
![](assets/atacama/alpha-shannon-transect.png)
...
![](../../../16s-atacama-tutorial/results/figures/pub_alpha_shannon.png)

**图 5：发表级 Shannon alpha 多样性箱线图。** Baquedano 的中位数高于 Yungay，与 QIIME2 内置检验结果一致（Kruskal-Wallis p = 0.042）。jitter 点展示了每个样本的实际数值。
...
![](assets/atacama/pcoa-bray-transect.png)
...
![](assets/atacama/pcoa-bray-transect-pub.png)
...
echo "  PC1 (9.37%): 主轴分离 Baquedano vs Yungay"
echo "  PC2 (6.10%): 反映组内与 site-level 差异"
echo ""
...
| 第 五 篇 | 多样性分析——你的样本有多"丰富"，彼此有多"不同" | **📍 本篇** |
