...
![](../../../16s-atacama-tutorial/results/figures/ch10_rf_importance.png)
...
![](../../../16s-atacama-tutorial/results/figures/ch10_rf_shap_beeswarm.png)
...
```r
# 第 7 篇中 ANCOM-BC 未检出显著差异属，LEfSe ∩ ANCOM-BC 交集为空
# 因此直接比较 RF 重要性排名与 LEfSe 显著差异属
lefse_genera <- c(“Crossiella”, “0319-7L14”, “Ammoniphilus”, “Marmoricola”,
                  “Blastococcus”, “JG30-KF-CM45”, “TK10”, “Paenibacillus”,
                  “Conexibacter”, “AKIW781”, “P2-11E”)

rf_top15 <- importance_df %>% slice_head(n = 15) %>% pull(Genus)
shared <- intersect(lefse_genera, rf_top15)
core_biomarkers <- tibble(Genus = shared)
write_tsv(core_biomarkers, file.path(ATACAMA_ROOT, “results”, “rf_core_biomarkers.tsv”))
cat(“LEfSe 显著 genus 中同时进入 RF 前 15 名的：\n”)
print(core_biomarkers)
```

```text
📊 输出：
LEfSe 显著 genus 中同时进入 RF 前 15 名的：
# A tibble: 8 × 1
  Genus
  <chr>
1 JG30-KF-CM45
...
这一步非常有价值。虽然第 7 篇中 ANCOM-BC 没有检出任何显著差异属，但 LEfSe 的 11 个候选 genus 中有 8 个出现在随机森林重要性前 15 名中——两种完全不同思路的方法（效应量筛选 vs 分类贡献度）高度汇聚。

**`JG30-KF-CM45` 是最突出的交叉验证信号：**

1. LEfSe 显著（P.adj = 0.0008，LDA = 3.81）
2. 随机森林重要性排名第 1（MeanDecreaseAccuracy = 12.49）

这就是”候选 biomarker”真正开始站稳的地方——不是靠 ANCOM-BC 的单独验证，而是靠多种独立方法的汇聚。
...
**和 LEfSe 差异分析交叉验证后，`JG30-KF-CM45` 是当前最稳的核心 biomarker 候选——LEfSe 显著且 RF 重要性第 1。**
...
| 第 十 篇 | 随机森林 Biomarker 筛选——谁最能代表这个群落 | **📍 本篇** |
